...
#
# The algorithm itself is untouched, and deliberately excluded from ruff so
# that it stays diffable against upstream.
#
# NB: this module's imports are heavy on purpose -- it is imported from inside
# the op function body, never at op module scope.
...
from skimage.filters import gaussian, rank, sobel, threshold_multiotsu
from skimage.measure import label, regionprops
from skimage.morphology import dilation, disk, erosion, thin
from skimage.segmentation import find_boundaries, mark_boundaries, watershed
...
def report(*args):
    """Relay the algorithm's running commentary to whoever is listening."""
    progress(" ".join(str(arg) for arg in args))


def plot_img(*args, **kwargs):
    """Plotting is not available in a worker process."""
